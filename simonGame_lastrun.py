...
gram_legal = [[0, 1, 2], [0, 3, 2], [2, 1, 0], [2, 3, 0]]
gram_illegal = [[1, 2, 3], [3, 2, 1], [1, 0, 3], [3, 0, 1]]
if groupIdx == 0:
    currGram = gram_legal
else:
...
for iTrial in range(nTrial):
    ...
    for iWord in range(nWord):
        choose_from = [0, 1, 2, 3]
        if iWord == 0:
            curr_idx = random.choice(choose_from)
        else:
            choose_from.remove(curr_idx)
            curr_idx = random.choice(choose_from)
        wordSeq.append(curr_idx)
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Guenevere\\UCL\\Alpha lab\\SimonGame_NTIO\\simonGame_lastrun.py',
...
text_perm_idx = int(expInfo['perm_number']) - 1
...
thisExp.addData('text_perm_idx',text_perm_idx)
# check responses
